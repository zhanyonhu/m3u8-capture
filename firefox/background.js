...
async function loadM3u8Settings() {
  ...
  try {
    const local = await ifdStorageGet('local', ['ifd_m3u8_prefs', 'ifd_bridge_port']);
    let p = local.ifd_m3u8_prefs && typeof local.ifd_m3u8_prefs === 'object' ? local.ifd_m3u8_prefs : null;
    let needPersist = false;
    if (!p) {
      let sync = {};
      try {
        sync = await ifdStorageGet('sync', ['ifd_m3u8_prefs', 'ifd_bridge_port']);
      } catch (_) {}
    ...
    if (needPersist) {
      ...
      await ifdStorageSet('local', { ifd_m3u8_prefs: merged, ifd_bridge_port: IFD_BRIDGE_PORT });
    }
  } catch (_) {
    IFD_BRIDGE_PORT = IFD_DEFAULT_BRIDGE_PORT;
    IFD_SEND_USER_AGENT = false;
...
async function buildOutboundHeadersForUrl(url, referer, tabId) {
  const headers = {};
  ...
  if (IFD_SEND_COOKIE) {
    const cookieHeader = await ifdBuildCookieHeader(url, referer, tabId);
...
function registerM3u8SettingsSyncListeners() {
  try {
    ...
    storageApi.onChanged.addListener((changes, areaName) => {
      ...
      if (changes.ifd_m3u8_prefs || changes.ifd_bridge_port) {
        ensureM3u8SettingsLoaded(true).catch(() => {});
      }
    });
  } catch (_) {}
}
...
try {
  ...
  console.error('[IFD-M3U8] webRequest.onBeforeRequest registration failed:', e);
}
...
function storageLocalSet(obj) {
  return ifdStorageSet('local', obj);
}
  ...
  ifdRebuildContextMenus(async () => {
    ...
    try {
      await ifdCreateContextMenuItem(
        { ...m3u8LinkBase, targetUrlPatterns: ['*://*/*m3u8*', '*://*/*.m3u8*'] },
        'IFD-M3U8'
      );
    } catch (_) {
      await ifdCreateContextMenuItem(m3u8LinkBase, 'IFD-M3U8');
    }
    await ifdCreateContextMenuItem({
...
async function bootstrapM3u8Extension() {
  try {
    await loadM3u8Settings();
    const local = await ifdStorageGet('local', ['ifd_m3u8_prefs']);
    if (!local.ifd_m3u8_prefs) {
    ...
    console.warn('[IFD-M3U8] bootstrap settings failed:', e);
  }
  registerM3u8SettingsSyncListeners();
}
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  (async () => {
    ...
    if (info.menuItemId === 'ifd-download-m3u8-link' && info.linkUrl) {
      if (!isM3u8Url(info.linkUrl)) return;
      const referer = (tab && tab.url) ? tab.url : '';
      const filename = deriveFilenameFromUrl(info.linkUrl, 'video');
      const headers = await buildOutboundHeadersForUrl(info.linkUrl, referer, tab && tab.id);
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'ifd_capture_download' && msg.url) {
    (async () => {
      ...
      const headers = await buildOutboundHeadersForUrl(msg.url, ref, tabId);
  ...
  if (msg.type === 'ifd_capture_batch_download' && Array.isArray(msg.items)) {
    (async () => {
      ...
      for (const it of msg.items) {
        ...
        const tabId = sender && sender.tab && typeof sender.tab.id === 'number' ? sender.tab.id : undefined;
        const headers = await buildOutboundHeadersForUrl(url, referer, tabId);
