...
async function loadMergedPrefs() {
  ...
  if (!p) {
  const sync = await ifdStorageGet('sync', ['ifd_m3u8_prefs', 'ifd_bridge_port']).catch(() => ({}));
...
async function persistPrefs(prefs) {
  ...
  try {
    await ifdStorageSet('sync', { ifd_m3u8_prefs: prefs, ifd_bridge_port: prefs.bridgePort });
  } catch (_) {}
  try {
...
loadMergedPrefs()
  ...
  .catch(() => {
    ...
    sendUaEl.checked = false;
    sendCookieEl.checked = false;
    sendRefererEl.checked = false;
  });
